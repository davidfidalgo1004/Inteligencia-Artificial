...
to Modo_Map
  if xcor = min-pxcor[ ;chega borda esquerda
    ifelse patch-ahead 1 = nobody and ycor != max-pycor[ ; se é parede
      set heading 0
    ][
      set heading 90
    ]
  ...
  if xcor = max-pxcor[ ;chega borda direita
    ifelse patch-ahead 1 = nobody[ ; se é parede
      set heading 0
    ][
      set heading -90
    ]
...
to go_once
  ;;atualizar probabilidades dos sliders enquanto simulação roda
  ask polluter 1 [
    set prob_sujar polluter_1_prob_sujar
  ]
  ask polluter 2 [
