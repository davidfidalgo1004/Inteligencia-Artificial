...
to go_once
  ...
  ask cleaners[
    ...
    let ajudame who
    ;;este é o metodo de carregar caso esteja no ponto de carregamento
    if battery = 0 []
    ask patch-here[
      ifelse pcolor = black and ([battery] of cleaner ajudame < cleaner_max_battery) [; "já chegamos" método, verifica se já está no ponto de carregamento
        ask cleaners[
        ...
        ask cleaners[
          ifelse battery <= (cleaner_max_battery / 1.8)[;; dirigir ao posto de carregamento
            if last_cleaning_location = [0 0][;; aspirador guarda sitio onde estava a aspirar até ter de ir carregar bateria
              ...
              if ticks > 10000 [set last_cleaning_location [-15 -15]]; senao ele fica la em cima e nao volta.... porque nao tem movimentos random suficiente para voltar para baixo
            ]
            ...
            ifelse capacity >= cleaner_max_capacity[
              ;; movimento depositar
              show "Capacidade Atingida"
              set battery battery - 1
            ][
              ;;movimento aspirar (fase inicial)
              ...
              ask patch-here[
                ;;encontrar residuo
                if pcolor != 39 and pcolor != black[ ;se estiver em cima de lixo
                  set pcolor 39 ; limpa
                  ask turtles[
                    if capacity < cleaner_max_capacity [set capacity capacity + 1 ]; armazena (if de segurança)
                  ]
                ]
